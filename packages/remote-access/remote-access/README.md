...
Adapted from `@deepseek-ai/dsh-remote-access` in the [DeepSeek Harness](https://github.com/deepseek-ai/dsh) source (MIT), published here under the `@froststarinquire` scope.
...
The plugin's scope is **connect only** — it establishes and maintains a tunnel, and deliberately does **not** add an authentication layer in front of the harness web service. A remote device that reaches the tunnel passes the `/api` browser-trust fence once `--trusted-host` whitelists the tunnel authority; pairing auth is the separate layer of the official harness that proves caller identity. The plugin is **inert unless a real tunnel credential is configured**: its `provider` defaults to `none`, so an unconfigured mount spawns nothing.
...
- **Connect only, no authentication** — the plugin opens a tunnel but does not gate who reaches the harness through it. Layer pairing auth or a trusted tunnel separately.
